      ...
      <div className="course-flow-form-grid">
        ...
        <label className="course-flow-field course-flow-field--wide">
          ...
          <textarea
            ...
            onChange={(event) =>
              onChange("fullDescription", event.target.value)
            }
          />
        </label>
      </div>
...
function LearningInfoForm({ form, onChange }) {
  return (
    <section className="course-flow-form-section">
      <h2>Basic Learning Setup</h2>
      <div className="course-flow-form-grid">
        <label className="course-flow-field course-flow-field--wide">
          <span>Target learners</span>
          <textarea
            rows="3"
            value={form.targetLearners}
            onChange={(event) => onChange("targetLearners", event.target.value)}
          />
        </label>
        <label className="course-flow-field course-flow-field--wide">
        ...
        <label className="course-flow-field course-flow-field--wide">
          ...
          <textarea
            ...
            onChange={(event) =>
              onChange("learningOutcomes", event.target.value)
            }
          />
        </label>
      </div>
    ...
    <section className="course-flow-form-section">
      ...
      <div className="course-flow-form-grid">
        <label className="course-flow-field">
          ...
          <select
            value={form.assignedSmeId}
            onChange={(event) => onChange("assignedSmeId", event.target.value)}
          >
        ...
        <label className="course-flow-field">
          ...
          <input
            ...
            onChange={(event) =>
              onChange("expectedCompletionDate", event.target.value)
            }
          />
        </label>
      </div>
...
function CoursePreviewCard({ form, previewStatus }) {
  const outcomes = useMemo(
    () => splitLines(form.learningOutcomes),
    ...
    <aside className="course-flow-preview-card">
      <div className="course-flow-preview-thumb">
        {form.thumbnail ? (
          ...
          <span>SLP Course</span>
        )}
      </div>
      <CourseStatusBadge status={previewStatus} />
      <h2>{form.title || "Course title preview"}</h2>
      <p>
        {form.shortDescription || "Short course description will appear here."}
      </p>
      <dl>
        <div>
          <dt>Category</dt>
        ...
        <div>
          ...
          <dd>
            {form.price
              ? `${Number(form.price).toLocaleString("vi-VN")} VND`
              : "Free"}
          </dd>
        </div>
      </dl>
      <div className="course-flow-preview-list">
        ...
        {outcomes.length ? (
          ...
          <span>Add outcomes for preview.</span>
        )}
      </div>
    </aside>
  );
...
export function TmoCreateCoursePage() {
  ...
  return (
    <section>
      <PageHeader
        title="Create Course"
        description="Create only basic course information and assign SME content ownership."
        action={
          <button
            type="button"
      ...
      <div className="course-flow-create-layout">
        <form
          className="course-flow-form-card"

      ...
      <header className="preview-lesson-content__head">
        ...
        <div>
          ...
          <div className="preview-lesson-content__meta">
            <span>
              <Clock3 size={13} /> {formatDuration(lesson.durationSeconds)}
            </span>
            {lesson.lessonType && (
...
export function CoursePreviewLessonsPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      setError(null);
      try {
        const data = await learningService.getPreviewContent(courseId, classId);
        if (cancelled) return;
        setCourse(
          data
            ? {
        ...
        const firstPreviewLesson = allLessons.find(
          (lesson) => lesson.isPreview === true,
        );

        setActiveId(firstPreviewLesson?.lessonId ?? null);
      } catch (err) {
        if (cancelled) return;
        const message =
          err?.message || "Could not load preview content for this course.";
        setError(message);
        toast.error(message);
      } finally {
    ...
    <div className="admin-page">
      ...
      {loading ? (
        ...
        <div className="admin-empty">
          This course does not have any preview lessons yet. Mark lessons as
          preview from the content management section.
        </div>
      ) : (
              ...
              >
                ...
                <ul className="preview-section__list">
                  {section.lessons.map((lesson) => {
                    ...
                    const isActive =
                      canPreview && lesson.lessonId === activeLesson?.lessonId;
                      ...
                      <li key={lesson.lessonId}>
                        <button
                          ...
                          className={
                            ...
                            (isActive ? " preview-section__item--active" : "") +
                            (!canPreview
                              ? " preview-section__item--locked"
                              : "")
                          }
                          disabled={!canPreview}
                          ...
                          title={
                            canPreview
                              ...
                              : "Enroll in this course or class to access this lesson"
                          }
                          onClick={() => {
                            if (canPreview) {
                              setActiveId(lesson.lessonId);
                            }
                          }}
                        >
                          ...
                          <span className="preview-section__copy">
                            ...
                            <small>
                              ...
                              {" · "}
                              {canPreview ? "Preview available" : "Locked"}
                            </small>
                          </span>
                        </button>
                      </li>

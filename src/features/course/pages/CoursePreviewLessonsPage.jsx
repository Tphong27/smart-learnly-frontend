...
function groupBySection(lessons) {
  ...
  for (const section of sections) {
    section.lessons.sort((a, b) => (a.lessonSortOrder ?? 0) - (b.lessonSortOrder ?? 0))
  }
  return sections
}
  ...
  return (
    <div className="preview-lesson-content">
      <header className="preview-lesson-content__head">
        ...
        <div>
          <h2 className="preview-lesson-content__title">{lesson.title}</h2>
          <div className="preview-lesson-content__meta">
            <span><Clock3 size={13} /> {formatDuration(lesson.durationSeconds)}</span>
            {lesson.lessonType && <span className="preview-chip">{lesson.lessonType}</span>}
          </div>
        </div>
      </header>

      {type.includes('video') && lesson.videoUrl && (
        <div className="preview-lesson-content__video">
          <video controls src={lesson.videoUrl} />
        </div>
      )}

      {lesson.content && (
        <div className="preview-lesson-content__body">
      ...
      {lesson.attachmentUrl && (
        <a className="preview-lesson-content__attachment" href={lesson.attachmentUrl} target="_blank" rel="noopener noreferrer">
...
export function CoursePreviewLessonsPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      setError(null)
      try {
        const data = await previewLessonService.list(courseId)
        if (cancelled) return
        setLessons(data || [])
        if (data && data.length > 0) {
          setActiveId(data[0].lessonId)
        }
      } catch (err) {
        if (cancelled) return
        const message = err?.message || 'Could not load preview content for this course.'
        setError(message)
        toast.error(message)
      } finally {
    ...
    <div className="admin-page">
      ...
      {loading ? (
        ...
        <div className="admin-empty">
          This course does not have any preview lessons yet. Mark lessons as preview from the content management section.
        </div>
      ) : (
        <div className="preview-layout">
          <aside className="preview-layout__sidebar">
            {sections.map((section) => (
              <div key={section.sectionId || 'no-section'} className="preview-section">
                <div className="preview-section__header">
                  Section {section.sortOrder ? section.sortOrder : ''}
                </div>
                  ...
                  {section.lessons.map((lesson) => {
                    ...
                    return (
                      <li key={lesson.lessonId}>
                        <button
                          type="button"
                          className={'preview-section__item' + (isActive ? ' preview-section__item--active' : '')}
                          onClick={() => setActiveId(lesson.lessonId)}
                        >
                          <span className="preview-section__icon">
                            <LessonIcon type={lesson.lessonType} />
                          </span>
                          <span className="preview-section__copy">
                            <span className="preview-section__title">{lesson.title}</span>

...
import { formatDateTime, shortId } from "@/shared/utils/formatters";
...
function ChangeHistoryDetailModal({ courseId, auditLogId, open, onClose }) {
    ...
    useEffect(() => {
        ...
        async function loadDetail() {
            ...
            try {
                const data = await courseChangeHistoryService.get(
                    courseId,
                    auditLogId,
                );
    ...
    return (
        <Modal open={open} title="Change detail" size="lg" onClose={onClose}>
            {loading ? (
    ...
    const columns = useMemo(
        () =>
            [
                ...
                ({ key }) => !["action", "target", "actions"].includes(key),
            ),
        [],
    );
        ...
        <section className="admin-page">
            <header className="admin-page__header">
                ...
                >
                    ...
                    <div>
                        <h1 className="admin-page__title">Change history</h1>
                    </div>
                </div>
            </header>

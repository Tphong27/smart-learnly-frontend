...
} from "@/services/video-ai.service";
...
function friendlyRequestError(error, fallback) {
  ...
  if (responseStatus === 404) {
    return "This study guide is no longer available.";
  }
  if (responseStatus === 409) {
    return "This study guide changed in another session. Reload it and try again.";
  }
  return fallback;
}
...
export default function VideoAiReviewPage() {
  ...
  const isTrainer =
    Boolean(classId) || location.pathname.startsWith("/trainer/");
  ...
  const formDisabled =
    !isDraft || saving || publishing || flashcardLoading;
  ...
  const loadContent = useCallback(async () => {
    ...
    try {
      ...
      if (
        !mountedRef.current ||
        requestedRoute !== routeKeyRef.current ||
      ...
      if (loadError?.originalError?.response?.status === 404) {
        ...
        setError(
          friendlyRequestError(
            loadError,
            "We could not load the study guide. Try again.",
          ),
        );
      }
    } finally {
  ...
  const loadFlashcardTargets = useCallback(async () => {
    ...
    try {
      ...
      setTargetLessonId((current) => {
        const stillExists = targets.some(
          (target) =>
            String(target.lessonId ?? target.id) === String(current),
        );
        return stillExists
          ? current
          : String(targets[0]?.lessonId ?? targets[0]?.id ?? "");
      });
      ...
      setFlashcardTargetsError(
        friendlyRequestError(
          targetsError,
          "We could not load the available flashcard lessons. Try again.",
        ),
      );
    } finally {
      if (
        ...
        setFlashcardTargetsLoading(false);
      }
    }
  }, [routeKey, service]);
  ...
  useEffect(() => {
    ...
    const confirmLinkNavigation = (event) => {
      ...
      if (
        !window.confirm(
          "You have unsaved study guide changes. Leave without saving?",
        )
      ) {
        event.preventDefault();
  ...
  function buildPayload() {
    return {
      ...
      segments: content.transcriptSegments.map(
  ...
  async function save() {
    ...
    if (!content?.summary?.trim()) {
      ...
      showToast({ type: "error", message: "Add a summary before saving." });
      return null;
    }

    setSaving(true);
    try {
      const saved = await service.saveContent(content.id, buildPayload());
      ...
      if (mountedRef.current && actionRoute === routeKeyRef.current) {
        showToast({
          ...
          message: friendlyRequestError(
            saveError,
            "We could not save the study guide. Try again.",
          ),
        });
      }
      return null;
  ...
  async function publish() {
    ...
    try {
      ...
      showToast({
        type: "success",
        message: "Study guide published to learners.",
      });
    } catch (publishError) {
      if (mountedRef.current && actionRoute === routeKeyRef.current) {
        showToast({
          ...
          message: friendlyRequestError(
            publishError,
            "We could not publish the study guide. Try again.",
          ),
        });
      }
    } finally {
    ...
    try {
      ...
      let job = await service.createFlashcardJob(savedContent.id, {
        ...
        desiredCount: Math.min(
          30,
          Math.max(1, Number(flashcardCount) || 10),
        ),
        difficulty: "medium",
      });
      showToast({
        type: "success",
        message: "Flashcard creation started.",
      });
      ...
      for (
        ...
        attempt < 180 &&
        mountedRef.current &&
        actionRoute === routeKeyRef.current;
        attempt += 1
      ) {
        setFlashcardProgress(job.progress);
        if (job.status === "COMPLETED") {
          if (!job.setId || !job.targetLessonId) {
            throw new Error("Flashcard review destination is unavailable");
          }
      ...
      if (mountedRef.current && actionRoute === routeKeyRef.current) {
        throw new Error("Flashcard creation is taking longer than expected");
      }
    } catch (flashcardError) {
      if (mountedRef.current && actionRoute === routeKeyRef.current) {
        console.error(
          "Unable to create flashcards from the study guide:",
          flashcardError,
        );
        showToast({
          type: "error",
          message: "We could not create the flashcards. Try again.",
        });
      }
    } finally {
      if (mountedRef.current && actionRoute === routeKeyRef.current) {
  ...
  return (
    <main className="video-ai-review" id="video-ai-review-main">
      <header className="video-ai-review__header">
        <button
          type="button"
          className="video-ai-review__back"
          onClick={goBack}
        ...
        <div>
          <span className="video-ai-review__eyebrow">
            <Sparkles size={15} aria-hidden="true" />
            Study guide draft
          </span>
          <h1>Review study guide</h1>
          <p>Check and edit each section before sharing it with learners.</p>
        </div>
        {content && (
          <span
            className={`video-ai-status video-ai-status--${content.status.toLowerCase()}`}
          >
      ...
      {loading && (
        <div className="video-ai-review__state" role="status">
          <Loader2 className="video-ai-spin" aria-hidden="true" />
          Loading study guide...
        </div>
      )}
      ...
      {!loading && error && (
        <div
          className="video-ai-review__state video-ai-alert video-ai-alert--error"
          role="alert"
        >
      ...
      {!loading && !error && !content && (
        <div className="video-ai-review__state">
          <Sparkles size={32} aria-hidden="true" />
          <h2>No study guide yet</h2>
          <p>Return to the lesson and create one after the video is ready.</p>
      ...
      {content && !loading && !error && (
        <>
          <div
            className="video-ai-review__tabs"
            role="tablist"
          ...
          >
            {TABS.map((tab, index) => (
              <button
                ...
                aria-controls={`video-ai-${tab.key}-panel`}
          ...
          <section
            id={`video-ai-${activeTab}-panel`}
            role="tabpanel"
            className="video-ai-review__panel"
          >
            {activeTab === "summary" && (
              <div className="video-ai-form">
                <label htmlFor="video-ai-summary">Summary</label>
                <textarea
                  id="video-ai-summary"
                  rows="9"
                ...
                />
                <label htmlFor="video-ai-key-points">
                  Key points <span>one per line</span>
                </label>
                <textarea
                  id="video-ai-key-points"
                  rows="6"
            ...
            {activeTab === "chapters" && (
              <div className="video-ai-list">
                {content.chapters.length === 0 ? (
                  <p className="video-ai-empty">No chapters were created.</p>
                ) : (
                  content.chapters.map((chapter, index) => (
                    <article key={chapter.id}>
                      <div className="video-ai-list__time">
                      ...
                      />
                      <label htmlFor={`chapter-summary-${chapter.id}`}>
                        Chapter summary
                      </label>
                      <textarea
            ...
            {activeTab === "transcript" && (
              <div className="video-ai-transcript-edit">
                {content.transcriptSegments.length === 0 ? (
                  ...
                  content.transcriptSegments.map((segment) => (
                    <div key={segment.id}>
                      ...
                      <label
                        className="video-ai-sr-only"
                        htmlFor={`segment-${segment.id}`}
                      >
                        Transcript at {formatTimestamp(segment.startMs)}
                      </label>
                      <textarea
                        ...
                        onChange={(event) =>
                          updateContent({
                            ...content,
                            transcriptSegments:
                              content.transcriptSegments.map((item) =>
                                item.id === segment.id
          ...
          <section
            className="video-ai-review__flashcards"
            aria-labelledby="video-ai-flashcards-title"
          >
            <div>
              <h2 id="video-ai-flashcards-title">
                Create flashcard drafts
              </h2>
              <p>Review and approve the cards before learners can use them.</p>
              {flashcardLoading && (
                <p role="status" aria-live="polite">
                  Creating flashcards: {flashcardProgress}%
                </p>
              )}
            </div>

            {flashcardTargetsLoading ? (
              <p className="video-ai-empty" role="status">
                Loading flashcard lessons...
              </p>
            ) : flashcardTargetsError ? (
              <div className="video-ai-alert video-ai-alert--error" role="alert">
                <AlertCircle size={18} aria-hidden="true" />
                <span>{flashcardTargetsError}</span>
                <Button
                  variant="secondary"
                  size="sm"
                  onClick={loadFlashcardTargets}
                >
                  Try again
                </Button>
              </div>
            ) : flashcardTargets.length ? (
              <div className="video-ai-review__flashcard-controls">
                <label>
                  Flashcard lesson
                  <select
                    value={targetLessonId}
                    disabled={flashcardLoading || saving || publishing}
                    onChange={(event) =>
                      setTargetLessonId(event.target.value)
                    }
                  >
                    {flashcardTargets.map((target) => (
                      <option
                        key={target.lessonId ?? target.id}
                        value={target.lessonId ?? target.id}
                      >
                        {target.title ?? target.lessonTitle}
                      </option>
                    ))}
                  </select>
                </label>
              ...
              <p className="video-ai-empty">
                Create a flashcard lesson first, then return here to add cards
                from this study guide.
              </p>
            )}
          </section>

          <footer className="video-ai-review__actions">
            ...
            {isDraft && (
              <>
                <Button
                  ...
                  loading={saving}
                  loadingLabel="Saving..."
                  disabled={!dirty || publishing || flashcardLoading}
                >
                  Save draft
                </Button>
                <Button
                  ...
                  disabled={saving || flashcardLoading}
                >
                  Publish to learners
                </Button>
              </>

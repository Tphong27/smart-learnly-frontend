    ...
    <div className="course-detail">
      <div className="course-detail__hero">
        <div className="course-detail__hero-main">
          ...
          {course.category && (
            <Link to={`/?category=${course.category.slug}`} className="course-detail__chip">
          ...
          <div className="course-detail__meta">
            ...
            <span className="course-detail__meta-item">
              <BookOpen size={14} /> {modules.length} {modules.length === 1 ? 'module' : 'modules'}
            </span>
            <span className="course-detail__meta-item">
              <Clock3 size={14} /> {totalLessons} {totalLessons === 1 ? 'lesson' : 'lessons'}
            </span>
        ...
        <aside className="course-detail__sidecard">
          ...
          <div className="course-detail__sidecard-body">
            ...
            <ul className="course-detail__sidecard-list">
              <li><CheckCircle2 size={14} /> Lifetime access to course materials</li>
              <li><CheckCircle2 size={14} /> Sample lessons available before enrollment</li>
              <li><CheckCircle2 size={14} /> Updated curriculum aligned with industry needs</li>
            </ul>
          </div>
        </aside>
      </div>
              ...
              .map((mod, idx) => (
                <li key={mod.id} className="course-detail__module">
                  <div className="course-detail__module-head">
                    ...
                    <div>
                      <h3 className="course-detail__module-title">{mod.title}</h3>
                      <small>
                    ...
                    {(mod.lessons || [])
                      ...
                      .map((lesson) => (
                        <li key={lesson.id} className="course-detail__lesson">
                          ...
                          {lesson.preview && (
                            <span className="course-detail__lesson-tag">Preview</span>
                          )}
                        </li>
                      ))}
                  </ul>
      ...
      <section className="course-detail__cta-row">
        <div>
          ...
          <p>Try the free preview lessons before committing to the full course.</p>
        </div>
        <Link
          to={`/courses/${course.id || course.slug}/preview`}

...
function sortByOrder(left, right) {
  return (
    Number(left?.orderIndex ?? left?.displayOrder ?? 0) -
    Number(right?.orderIndex ?? right?.displayOrder ?? 0)
  );
}
...
function ReadOnlyQuizPreview({ lesson, courseId, classId, adminMode = false }) {
  ...
  useEffect(() => {
    ...
    async function loadPreviewQuestions() {
      try {
        ...
        const nextQuestions = Array.isArray(result)
          ? [...result].sort(sortByOrder).map((question) => ({
              ...question,
              answers: Array.isArray(question.answers)
                ? [...question.answers].sort(sortByOrder)
                : [],
            }))
          : [];
        ...
        setQuestions([]);
        setLoadError(
          requestError?.message || "Could not load preview questions.",
        );
      } finally {
        if (!cancelled) {
          setLoading(false);
  ...
  if (loading) {
    return (
      <div className="learning-quiz-preview__state">
        <Loader2
          className="learning-quiz-preview__spinner"
          size={24}
          aria-hidden="true"
        />

        <p>Loading preview questions...</p>
      </div>
    );
  }
  ...
  if (loadError) {
    return (
      <div className="learning-quiz-preview__state learning-quiz-preview__state--error">
  ...
  return (
    <section
      className="learning-quiz-preview"
      aria-labelledby="preview-test-title"
    >
      <header className="learning-quiz-preview__header">
        <div className="learning-quiz-preview__header-icon">
          <Eye size={22} aria-hidden="true" />
        </div>

        <div>
          <h2 id="preview-test-title">{lesson?.title || "Test preview"}</h2>

          <p>
            Questions and answer choices are shown for preview only. Enroll in
            the class to answer and submit this test.
          </p>
        </div>

        <span className="learning-quiz-preview__badge">
      ...
      {questions.length === 0 ? (
        <div className="learning-quiz-preview__empty">
          No questions are available for preview.
        </div>
      ) : (
        <ol className="learning-quiz-preview__questions">
          {questions.map((question, questionIndex) => {
            const questionType = String(
              question.questionType || "",
            ).toLowerCase();

            const inputType = questionType.includes("multiple_select")
              ? "checkbox"
              : "radio";

            return (
              <li
                key={`preview-question-${questionIndex}`}
                className="learning-quiz-preview__question"
              >
                ...
                {question.audioUrl && (
                  <audio
                    className="learning-quiz-preview__question-audio"
                    controls
                    preload="metadata"
                ...
                <div
                  className="learning-quiz-preview__answers"
                  aria-label={`Answer choices for question ${
                    questionIndex + 1
                  }`}
                >
                  {question.answers.map((answer, answerIndex) => (
                    <label
                      key={`preview-answer-${questionIndex}-${answerIndex}`}
                      className="learning-quiz-preview__answer"
                    >
                      <input
                        type={inputType}
                      ...
                      <span className="learning-quiz-preview__answer-marker">
      ...
      <footer className="learning-quiz-preview__footer">
        <LockKeyhole size={16} aria-hidden="true" />

        <span>
          Answer selection and submission are disabled in preview mode.
        </span>
      </footer>
    </section>
  );
          ...
          {hasResult && (
            <div className="assignment-result">
              <div className="assignment-result__score">
                ...
                <span>
                  {gradedAt ? `Graded ${gradedAt}` : "Review complete"}
                </span>
              </div>

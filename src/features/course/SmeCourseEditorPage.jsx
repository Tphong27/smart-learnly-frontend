    ...
    <Modal
      ...
      footer={
        <div className="course-flow-modal-actions">
          ...
          <button type="button" className="demo-primary-action" onClick={handleSubmit}>
            Add Module
          </button>
        </div>
      }
...
function AddLessonModal({ modules, selectedModuleId, open, onClose, onSubmit }) {
...
function CourseStructurePanel({
  ...
  return (
    <aside className="course-editor-panel course-editor-outline">
      ...
      {course.status === COURSE_STATUSES.REVISION_REQUIRED && course.revisionReason ? (
          ...
          {modules.map((module) => (
            <article key={module.id}>
              <div className="course-builder-module-header">
                ...
                <span>
                  <button type="button" title="Edit module mock">
                    <Edit3 size={13} />
                  </button>
                  <button type="button" title="Delete module mock">
                    <Trash2 size={13} />
                  </button>
                </span>
              </div>
              ...
              <div>
                {module.lessons.length === 0 ? (
                  ...
                  <button
                    key={lesson.id}
                    type="button"
                    className={lesson.id === selectedLessonId ? 'is-active' : ''}
                    onClick={() => onSelectLesson(lesson.id)}
                  >
                    <span>{lesson.title}</span>
                    <small>
                      {lesson.type} | {lesson.durationMinutes || lesson.duration || 0} min | {lesson.status}
                    </small>
                  </button>
                ))}
              </div>
      ...
      <div className="course-editor-outline-actions">
        <button type="button" className="demo-secondary-action" onClick={onAddModule}>
          <Layers3 size={16} />
          Add Module
        </button>
        <button type="button" className="demo-secondary-action" onClick={onAddLesson}>
          <BookOpen size={16} />
          Add Lesson
        </button>
      </div>
...
function MockVideoUploader({ courseId, lesson, onUploaded }) {
  ...
  return (
    <section className="course-builder-upload-box">
      ...
      <button type="button" className="demo-secondary-action" onClick={uploadVideo}>
        <Upload size={16} />
        Upload Video Mock
      </button>
      <div className="course-flow-resource-list">
        {uploadedVideos.length === 0 ? (
          ...
          <article key={video.id}>
            ...
            <small>{video.size} | {new Date(video.uploadedAt).toLocaleString('vi-VN')}</small>
          </article>
        ))}
      </div>
    </section>
...
function UploadMaterialBox({ courseId, lesson, onUploaded }) {
  ...
  return (
    <section className="course-builder-upload-box">
      ...
      <button type="button" className="demo-secondary-action" onClick={uploadMaterial}>
        <Upload size={16} />
      ...
      <div className="course-flow-resource-list">
        {materials.length === 0 ? (
          ...
          <article key={material.id}>
            ...
            <small>{material.type} | {new Date(material.uploadedAt).toLocaleString('vi-VN')}</small>
          </article>
        ))}
      </div>
    </section>
  ...
  return (
    <main className="course-editor-panel course-editor-lesson">
      <section className="course-flow-form-section">
        ...
        <MockVideoUploader courseId={courseId} lesson={lesson} onUploaded={onRefresh} />
        ...
        <section className="course-builder-video-preview">
          ...
          <small>{form.videoUrl || 'Upload a video or add a URL to preview.'}</small>
        </section>

        <UploadMaterialBox courseId={courseId} lesson={lesson} onUploaded={onRefresh} />

        <label className="course-flow-field">
          <span>Rich text lesson content</span>
          <RichTextEditorMock value={form.content} onChange={(value) => updateForm('content', value)} />
        </label>
...
function GeneratedResourcesList({ resources }) {
  if (resources.length === 0) {
    return <p className="demo-muted">Generated summaries, flashcards, questions, tests, and key points will appear here.</p>
  }
...
function CourseBuilderToolsPanel({
  ...
  const aiActions = [
    ...
    },
    {
      ...
      icon: ClipboardCheck,
      action: () => generatePracticeTestForLesson(courseId, lessonId, ROLES.SME),
    },
    {
      label: 'Generate Quiz Questions',
      icon: FileQuestion,
      action: () => generateQuizQuestionsForLesson(courseId, lessonId, ROLES.SME),
    },
    {
  ...
  return (
    <aside className="course-editor-panel course-editor-tools">
      ...
      <button type="button" className="demo-primary-action" onClick={onSubmitReview}>
        <Send size={16} />
...
export function SmeCourseEditorPage() {
  ...
  const [selectedLessonId, setSelectedLessonId] = useState(() => getFirstLessonId(getLifecycleModules(courseId)))
  const [resources, setResources] = useState(() => getGeneratedResources({ courseId }))
  const [addModuleOpen, setAddModuleOpen] = useState(false)
  const [addLessonOpen, setAddLessonOpen] = useState(false)

  const selectedLesson = useMemo(() => {
    return modules.flatMap((module) => module.lessons).find((lesson) => lesson.id === selectedLessonId)
  ...
  if (!course) {
    return (
      <section>
        <PageHeader title="Course Builder Workspace" description="No matching course was found." />
        <DataState type="empty" title="Course not found" description="Open a course from Assigned Courses." />
      </section>
    )
  }
  ...
  const refreshResources = () => setResources(getGeneratedResources({ courseId }))
    ...
    <section>
      ...
      <div className="course-editor-layout course-builder-workspace">
        ...
        <CourseBuilderToolsPanel
          ...
          resources={resources.filter((resource) => !selectedLessonId || resource.lessonId === selectedLessonId)}

...
export function TeacherMonitorPage() {
            ...
            setAttemptHistory((current) => {
                const attempt = {
                    ...
                    studentName:
                        event.studentName ||
                        current.find(
                            (item) => item.studentId === event.studentId,
                        )?.studentName ||
            ...
            if (normalizedType === "essay") {
                ...
                const [test, attempts, questionMappings] = await Promise.all([
                    ...
                    testService.getStaffQuestions(id).catch((questionError) => {
                        console.warn(
                            "Could not load MCQ question total",
                            questionError,
                        );
                        return [];
                    }),
                ...
                setQuestionTotal(
                    getQuestionTotal({ questions: questionMappings }),
                );
    ...
    const handleViewMcqDetail = async (row) => {
        ...
        try {
            const answers = await attemptService.getStudentAnswers(
                row.attemptId,
            );
        ...
        try {
            ...
            setHistoryAnswerDetails((current) => ({
                ...
                [attemptId]: {
                    ...
                    loading: false,
                    error:
                        error.message || "Could not load this attempt detail.",
                },
            }));
        }
    };
                            ...
                            {rowList.map((row) => {
                                ...
                                return (
                                    <tr key={row.studentId}>
                                        ...
                                        <td>
                                            <span
                                                className={`ft-badge ${info.className}`}
                                            >
                                                {info.className ===
                                                "ft-status--expired" ? (
                                                    <XCircle size={14} />
                                    ...
                                    (best, attempt) => {
                                        const score =
                                            getMcqScorePercentage(
                                                attempt,
                                                getQuestionTotal(attempt) ||
                                                    questionTotal,
                                            ) ?? -1;
                                        const bestScore =
                                            getMcqScorePercentage(
                                                best,
                                                getQuestionTotal(best) ||
                                                    questionTotal,
                                            ) ?? -1;
                    ...
                    <div className="ft-detail-dialog ft-detail-dialog--history">
                        ...
                        <div className="ft-attempt-detail-list">
                            {historyDetailRow.attempts.map((attempt, index) => {
                                ...
                                const isExpanded =
                                    expandedHistoryAttemptId === attemptId;
                                return (
                                    <div
                                    ...
                                    >
                                        <div className="ft-history-attempt__summary">
                                            ...
                                            <button
                                                ...
                                                onClick={() =>
                                                    handleToggleHistoryAttemptDetail(
                                                        attempt,
                                                    )
                                                }
                                            >
                                        ...
                                        {isExpanded && (
                                            <div className="ft-history-answer-panel">
                                                {historyAnswerDetails[attemptId]
                                                    ?.loading ? (

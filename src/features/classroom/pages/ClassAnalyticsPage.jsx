...
function MetricCard({ icon, label, value, helper }) {
    return (
        <article className="analytics-metric-card">
            <div className="analytics-metric-card__icon">{icon}</div>

            <div>
                <p className="analytics-metric-card__label">{label}</p>

                <strong className="analytics-metric-card__value">
                    {value}
                </strong>

                {helper && (
                    <p className="analytics-metric-card__helper">{helper}</p>
                )}
            </div>
        </article>
    );
    ...
    useEffect(() => {
        ...
        async function loadAnalytics() {
            try {
                ...
                const data =
                    role === ROLES.TRAINER
                        ? await classAnalyticsService.getTrainer(
                              classId,
                              inactiveDays,
                          )
                        : await classAnalyticsService.getAdmin(
                              classId,
                              inactiveDays,
                          );
    ...
    return (
        <section className="class-analytics-page">
            <header className="class-analytics-header">
                ...
                <div>
                    <p className="class-analytics-header__eyebrow">
                        Class Analytics
                    </p>
            ...
            <section className="class-analytics-summary">
                <MetricCard
                    icon={<Users size={22} />}
                ...
                <MetricCard
                    ...
                    helper={`No activity for ${inactiveDays}+ days`}
                />
            </section>
            ...
            <section className="analytics-panel">
                <div className="analytics-panel__header">
                    <div>
                        ...
                        <p>
                            Learning progress, assessment performance, and
                            recent activity for active class members.
                        </p>
                    </div>
                        ...
                        <tbody>
                            {sortedStudents.map((student) => (
                                <tr key={student.studentId}>
                                    ...
                                    <td>
                                        {formatNumber(student.averageTestScore)}
                                    </td>
                                    <td>
                                        {formatNumber(

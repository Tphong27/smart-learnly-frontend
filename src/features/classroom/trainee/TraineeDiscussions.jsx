...
import { FileUp, Paperclip, Pin, Plus, Send } from 'lucide-react'
...
export function TraineeDiscussions() {
  ...
  return (
    <div>
      ...
      <FilterToolbar>
        <SearchBox value={filters.keyword} placeholder="Search posts and topics" ariaLabel="Search class posts" onChange={(v) => updateFilter('keyword', v)} />
        <SelectFilter value={filters.pinned} onChange={(v) => updateFilter('pinned', v)} ariaLabel="Filter pinned" disabled={tab !== 'announcements'}
          options={[
        ...
        />
        <SelectFilter value={filters.sort} onChange={(v) => updateFilter('sort', v)} ariaLabel="Sort posts"
          options={[
          ...
          {creatingDisc ? (
            <div className="classflow-submission-panel" style={{ marginBottom: '1rem' }}>
              ...
              <label className="course-flow-field" style={{ marginTop: '0.5rem' }}>
                <span>Content</span>
                <textarea rows="5" value={discForm.content} onChange={(e) => setDiscForm((p) => ({ ...p, content: e.target.value }))} placeholder="Write your post content..." />
              </label>

...
export function OpeningScheduleDetailPage() {
  ...
  const backTarget =
    location.state?.from || "/opening-schedule";
  ...
  useEffect(() => {
    ...
    async function loadDetail() {
      ...
      try {
        const result =
          await openingScheduleService.getDetail(classId);
  ...
  async function handleRegister() {
    ...
    if (
      String(classItem.status || "").toUpperCase() !==
      "UPCOMING"
    ) {
      toast.error(
    ...
    try {
      ...
      if (classPrice === 0) {
        const enrollment =
          await enrollmentService.enrollFreeClass(
            classItem.classId,
          );
        ...
        if (enrollment?.alreadyEnrolled) {
          ...
          toast.success(
            "Your class enrollment has been reactivated.",
          );
        } else {
          toast.success(
            "Class enrollment completed.",
          );
        }
      ...
      const checkout =
        await checkoutService.checkoutClass(
          classItem.courseId,
          classItem.classId,
        );
      ...
      toast.error(
        requestError?.message ||
          "Could not register for this class.",
      );
    } finally {
      setSubmitting(false);
    }
    ...
    return (
      <main className="opening-detail">
        <div className="opening-state opening-state--error">
          ...
          <p>
            {error || "Opening class was not found."}
          </p>
  ...
  const hasValidClassPrice =
    Number.isFinite(normalizedClassPrice) &&
    normalizedClassPrice >= 0;
  ...
  const courseTitle =
    classItem.courseTitle || "Associated course";

  const courseDescription =
    classItem.courseDescription || "";

  const courseCategoryName =
    classItem.courseCategoryName || "Uncategorized";

  const courseLevel =
    classItem.courseLevel || "Not specified";
      ...
      <div className="opening-detail__hero">
        <div className="opening-detail__hero-main">
          ...
          <section className="opening-detail__hero-card">
            ...
            <span className="opening-detail__eyebrow">
              Offline class
            </span>

            <h1 className="opening-detail__title">
              {classItem.className}
            </h1>
          </section>
            ...
            <div className="opening-detail__section-head">
              <div>
                ...
                <p className="opening-detail__section-sub">
                  Review the trainer, class duration,
                  availability and weekly schedule before
                  registering.
                </p>
              </div>
            </div>
          ...
          <section className="opening-detail__section">
            ...
            <article className="opening-detail__course-overview">
              <header className="opening-detail__course-header">
                <div className="opening-detail__course-heading">
                  <span className="opening-detail__course-category">
                    {courseCategoryName}
                  </span>

                  <h3 className="opening-detail__course-title">
                    {courseTitle}
                  </h3>
                </div>
              </header>

              <dl className="opening-detail__course-facts">
                <div className="opening-detail__course-fact">
                  <dt>Category</dt>
                  <dd>{courseCategoryName}</dd>
                </div>
              ...
              <div className="opening-detail__course-about">
                ...
                {courseDescription ? (
                  ...
                  <p className="opening-detail__course-empty">
          ...
          <div className="opening-detail__sidecard-body">
            ...
            {classItem.courseId &&
              classItem.classId && (
                <Link
                  to={
                    `/courses/${classItem.courseId}/preview` +
                    `?classId=${classItem.classId}`
                  }
                  className="opening-button opening-button--secondary opening-detail__preview-button"
                >
                  <Eye
                    size={17}
                    aria-hidden="true"
            ...
            <ul className="opening-detail__sidecard-list">
              ...
              <li>
                <CheckCircle2
                  ...
                  aria-hidden="true"
                />
                Class capacity is limited to{" "}
                {classItem.maxStudents} learners
              </li>
            </ul>
          </div>
        </aside>

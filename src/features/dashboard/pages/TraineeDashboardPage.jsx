...
import { courseService, traineeProgressService } from "@/services";
import { TraineeDashboardSkeleton } from "../components/TraineeDashboardSkeleton";
import { TraineeProgress } from "@/features/progress";
...
function enrichCourse(course, progressMap) {
  ...
  return {
    ...
    categoryName: course.category?.name || progress.categoryName || "Course",
  ...
  return (
    ...
    >
      <svg viewBox="0 0 44 44" aria-hidden="true">
        <circle
          className="trainee-dashboard-momentum__ring-track"
          cx="22"
          cy="22"
          r="18"
        />
...
export function TraineeDashboardPage() {
  ...
  useEffect(() => {
    ...
    async function loadDashboard() {
      ...
      if (
        ...
        setError(failure?.message || "Could not load your learning dashboard.");
        setCourses([]);
        setProgress(null);
      } else {
  ...
  const dashboard = useMemo(() => {
    ...
    const completedCourses =
      progress?.completedCourses ?? enrolledCourses.filter(isCompleted).length;
    ...
    const lessonTotals = enrolledCourses.reduce(
      (total, course) => ({
        completed: total.completed + toNumber(course.lesson?.completed),
        lessons: total.lessons + toNumber(course.lesson?.total),
      }),
      { completed: 0, lessons: 0 },
    );
    const lessonProgress = lessonTotals.lessons
      ? Math.round((lessonTotals.completed / lessonTotals.lessons) * 100)
      : overallProgress;
  ...
  return (
    <main className="trainee-dashboard-page">
      ...
      {!loading && !error && courses.length > 0 && (
        ...
        >
          <div className="trainee-dashboard-momentum__intro">
            ...
            <div>
              <h2 id="learning-momentum-title">Keep your learning momentum</h2>
              <p>
          ...
          <div className="trainee-dashboard-momentum__progress">
            ...
            <div>
              <strong>
                {dashboard.lessonCompleted}/{dashboard.lessonTotal} lessons
              </strong>
              <span>completed</span>
            </div>
          </div>
        ...
        >
          ...
          <div className="trainee-dashboard-feature__content">
            <span className="trainee-dashboard-feature__eyebrow">
              <CirclePlay size={16} aria-hidden="true" /> Continue learning
            </span>
            ...
            <div className="trainee-dashboard-feature__progress-copy">
              <span>{getItemCount(dashboard.featuredCourse)}</span>
              <strong>
                {dashboard.featuredCourse.overallPercent}% complete
              </strong>
            </div>
            <ProgressLine value={dashboard.featuredCourse.overallPercent} />
            ...
            <div className="trainee-dashboard-feature__actions">
              {dashboard.featuredCourse.accessAllowed ? (
                <Button
                  to={getLearningPath(dashboard.featuredCourse)}
      ...
      {!loading && !error && progress && (
        <TraineeProgress progress={progress} />
      )}

...
export function PersonalFlashcardSetDetailPage() {
        ...
        async (showLoading = true) => {
            ...
            try {
                ...
                setError(
                    getErrorMessage(
                        loadError,
                        "Unable to load this flashcard set.",
                    ),
                );
                return null;
            } finally {
    ...
    async function saveSet(values) {
        const updated = await personalFlashcardService.replaceSet(
            setId,
            values,
        );
    ...
    async function bulkCreateCards(cards) {
        const updated = await personalFlashcardService.bulkCreateCards(
            setId,
            cards,
        );
        setFlashcardSet({ ...updated, cards: normalizeCards(updated?.cards) });
            ...
            <header className="personal-flashcards-page__header personal-flashcards-page__header--detail">
                <div>
                    ...
                    <span className="personal-flashcards-page__metadata">
                        ...
                        {flashcardSet.cards.length === 1 ? "card" : "cards"}
                        <span aria-hidden="true">{" \u00b7 "}</span>
                        Updated{" "}
                        {formatPersonalFlashcardDate(flashcardSet.updatedAt)}
                    </span>
                </div>

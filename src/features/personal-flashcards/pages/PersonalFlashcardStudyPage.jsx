...
import { getApiStatus, getErrorMessage, normalizeCards } from "../utils/personal-flashcard-utils";
...
export function PersonalFlashcardStudyPage() {
  ...
  const loadStudy = useCallback(async () => {
    ...
    try {
      ...
      if (mountedRef.current) setStudy({ ...data, cards: normalizeCards(data?.cards) });
    } catch (loadError) {
      ...
      setError(getErrorMessage(loadError, "Unable to load this study set."));
    } finally {
  ...
  if (loading) {
    return <section className="personal-flashcards-page"><div className="personal-flashcard-state" role="status">Loading study set...</div></section>;
  }
    ...
    return (
      <section className="personal-flashcards-page">
        <div className="personal-flashcard-state personal-flashcard-state--error" role="alert">
          <p>{error}</p>
          <Button variant="secondary" leftIcon={<RefreshCw size={16} aria-hidden="true" />} onClick={() => void loadStudy()}>
            Try again
          </Button>
        </div>
      </section>
  ...
  return (
    <section className="personal-flashcards-page personal-flashcards-page--study">
      <Link to={`/flashcards/${setId}`} className="personal-flashcards-back-link">
      ...
      <header className="personal-flashcards-page__header personal-flashcards-page__header--study">
        <div>
          <span className="personal-flashcards-page__eyebrow">Study session</span>
          <h1>{study.title}</h1>
          <p>Flip each card, then move through the set at your own pace.</p>
        </div>
      </header>

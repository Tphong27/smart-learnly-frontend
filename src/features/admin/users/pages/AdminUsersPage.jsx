...
function getUserFormSchema(mode) {
  return z
    .object({
      ...
      email: z.string().trim().email("Email is invalid").max(255, "Email must be at most 255 characters"),
      avatarUrl: z.string().trim().max(500, "Avatar URL must be at most 500 characters").or(z.literal("")).optional(),
      phoneNumber: z.string().trim().max(20, "Phone number must be at most 20 characters").or(z.literal("")).optional(),
      bio: z.string().max(5000, "Bio must be at most 5000 characters").or(z.literal("")).optional(),
...
function UserFormModal({ open, mode, initial, onClose, onSaved }) {
  ...
  return (
    ...
    >
      {serverError && <div className="auth-card__alert" style={{ marginBottom: 16 }}>{serverError}</div>}

      <Form onSubmit={handleSubmit(onSubmit)}>
        <div className="admin-form-grid">
          <FormField label="Full name" required registration={register("fullName")} error={errors.fullName?.message} />
          <FormField label="Email" required type="email" registration={register("email")} error={errors.email?.message} />

          <div className="input-field">
            <label className="input-field__label" htmlFor="admin-user-role">Role<span className="input-field__required">*</span></label>
            <select id="admin-user-role" className="admin-toolbar__select" {...register("role")}>
              {USER_ROLES.map((role) => <option key={role} value={role}>{role}</option>)}
            </select>
            {errors.role && <p className="input-field__error">{errors.role.message}</p>}
          </div>

          <div className="input-field">
            <label className="input-field__label" htmlFor="admin-user-status">Status<span className="input-field__required">*</span></label>
            <select id="admin-user-status" className="admin-toolbar__select" {...register("status")}>
              {USER_STATUSES.map((status) => <option key={status} value={status}>{formatLabel(status)}</option>)}
            </select>
            {errors.status && <p className="input-field__error">{errors.status.message}</p>}
          </div>

          <FormField label="Phone number" registration={register("phoneNumber")} error={errors.phoneNumber?.message} />
          <FormField label="Avatar URL" registration={register("avatarUrl")} error={errors.avatarUrl?.message} />

          <div className="admin-form-grid__full">
            <FormField
              label={mode === "edit" ? "Reset password" : "Password"}
              required={mode === "create"}
              type="password"
              registration={register("password")}
              error={errors.password?.message}
              helperText={mode === "edit" ? "Leave blank to keep the current password." : "At least 8 characters with uppercase, lowercase, number, and special character."}
            />
          </div>

          <div className="admin-form-grid__full">
            <div className="input-field">
              <label className="input-field__label" htmlFor="admin-user-bio">Bio</label>
              <textarea id="admin-user-bio" className="admin-textarea" rows={3} {...register("bio")} />
...
function DeleteUserModal({ open, target, onClose, onConfirmed }) {
  ...
  return (
    <Modal open={open} title="Delete user" size="sm" onClose={loading ? undefined : onClose}>
      <p style={{ margin: 0, color: "#475569", fontSize: 14, lineHeight: 1.6 }}>
        This will soft-delete <strong>{target?.fullName}</strong> and deactivate the account.
      </p>
              ...
              <tbody>
                {items.map((user) => (
                  <tr key={user.id}>
                    <td>
                      <div className="admin-user-cell">
                        <div className="admin-user-cell__avatar">
                          {user.avatarUrl ? <img src={user.avatarUrl} alt="" /> : <UserCog size={18} />}
                        </div>
                        <div>
                    ...
                    <td><span className={`admin-status admin-status--${user.status || "inactive"}`}>{formatLabel(user.status)}</span></td>
                    <td>{user.phoneNumber || <span style={{ color: "#94a3b8" }}>--</span>}</td>
                    ...
                    <td>
                      <div className="admin-table__actions" style={{ float: "right" }}>
                        <button
                          ...
                          title="Edit"
                          onClick={() => setFormState({ open: true, mode: "edit", initial: user })}
                        >
                          <Edit2 size={15} />
                        </button>
                        <button

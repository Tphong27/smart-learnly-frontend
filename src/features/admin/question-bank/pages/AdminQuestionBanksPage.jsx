...
function BankModal({ bank, open, courses, onClose, onSaved }) {
    ...
    async function handleSubmit(event) {
        ...
        try {
            ...
            setError(
                err?.message ||
                    `Could not ${editing ? "update" : "create"} question bank.`,
            );
        } finally {
            setSubmitting(false);
        }
    ...
    return (
        <Modal
            open={open}
            title={editing ? "Edit question bank" : "Create question bank"}
            size="md"
...
export function AdminQuestionBanksPage() {
    ...
    useEffect(() => {
        ...
        (async () => {
            ...
            try {
                ...
                if (!cancelled) {
                    const message =
                        err?.message || "Could not load question banks.";
                    setError(message);
                    toast.error(message);
                }
            } finally {
    ...
    const hasActiveFilters = Boolean(
        search.trim() || courseId || status !== "all",
    );
    ...
    async function handleArchive(bank) {
        ...
        const confirmed = window.confirm(
            `Archive question bank "${bank.name}"?`,
        );
        if (!confirmed) return;
        ...
        <div className="admin-page">
            ...
            <section className="admin-card admin-card--flush admin-card--filterable">
                <AdminFilterToolbar
                    ...
                    activeFilterCount={
                        Number(Boolean(courseId)) + Number(status !== "all")
                    }
                ...
                <div className="admin-table-wrap">
                    {loading ? (
                        ...
                        <div className="admin-empty">
                            No question banks match the current filters.
                        </div>
                    ) : (
                            ...
                            <tbody>
                                {items.map((bank) => (
                                    <tr key={bank.bankId || bank.id}>
                                        ...
                                        <td>
                                            <span
                                                className={`admin-status admin-status--${bank.status}`}
                                            >
                                                {bank.status}
                                            </span>
                                        </td>
                                        ...
                                        <td>
                                            ...
                                            >
                                                <Link
                                                    ...
                                                    title="Open"
                                                    to={`/admin/question-banks/${bank.bankId || bank.id}`}
                                                >
                                                    <Eye size={15} />
                                                </Link>
                                                {writable &&
    ...
    return (
        ...
        >
            <p style={{ marginTop: 0, color: "#475569" }}>
                Restore <strong>{bank?.name || "this question bank"}</strong> so
                it can be edited again. Choose the status to apply after
                restoring.
            </p>
            <div
                style={{

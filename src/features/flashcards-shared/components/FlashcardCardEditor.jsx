...
export function FlashcardCardEditor({
  ...
  const initialValue = isControlled ? controlledDraft : value;
  const [internalDraft, setInternalDraft] = useState(() => toDraft(value));
  const draft = isControlled ? toDraft(controlledDraft) : internalDraft;
  const [openSections, setOpenSections] = useState(() =>
    getOpenSections(toDraft(initialValue)),
  );
  const [uploadingField, setUploadingField] = useState(null);

  useEffect(() => {
    if (!isControlled) {
      onDraftChange?.(internalDraft);
    }
  }, [internalDraft, isControlled, onDraftChange]);
  ...
  const frontTextErrorId = `${frontTextId}-error`;
  const backTextErrorId = `${backTextId}-error`;
  const frontTextError = fieldErrors.frontText;
  const backTextError = fieldErrors.backText;
          ...
          <section className="flashcard-card-editor__side">
            <div className="flashcard-field">
              ...
              <textarea
                ...
                disabled={saving}
                aria-invalid={Boolean(frontTextError)}
                aria-describedby={frontTextError ? frontTextErrorId : undefined}
              />
              {frontTextError && (
                <p
                  id={frontTextErrorId}
                  className="flashcard-field__error"
                  role="alert"
                >
                  {frontTextError}
                </p>
          ...
          <section className="flashcard-card-editor__side">
            <div className="flashcard-field">
              ...
              <textarea
                ...
                disabled={saving}
                aria-invalid={Boolean(backTextError)}
                aria-describedby={backTextError ? backTextErrorId : undefined}
              />
              {backTextError && (
                <p
                  id={backTextErrorId}

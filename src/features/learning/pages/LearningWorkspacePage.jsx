...
/** Định dạng thời lượng lesson ngắn gọn để hiển thị trong giao diện học. */
...
/** Lấy id lesson tương thích với các payload curriculum cũ và mới. */
function getLessonId(lesson) {
  return lesson?.lessonId ?? lesson?.id ?? null;
}

/** Hiển thị biểu tượng theo loại nội dung lesson. */
...
/** Tạo khóa ổn định để lưu vị trí học riêng cho từng người dùng. */
function getUserPreferenceKey(user) {
  return user?.id || user?.userId || user?.accountId || user?.email || null;
}

/** Tạo khóa local storage cho lesson gần nhất của một course hoặc class. */
...
/** Đọc id lesson gần nhất từ local storage, an toàn khi trình duyệt chặn storage. */
...
/** Lưu id lesson gần nhất để lần sau học viên tiếp tục đúng vị trí. */
...
/** Tìm lesson hợp lệ trong danh sách hiện tại theo id. */
...
/** Hiển thị workspace học bài cho học viên, khách xem trước hoặc quản trị xem trước. */
export function LearningWorkspacePage({
  ...
  useEffect(() => {
    ...
    /** Đồng bộ trạng thái sidebar khi viewport chuyển giữa desktop và mobile. */
  ...
  useEffect(() => {
    ...
    /** Đóng sidebar di động khi người dùng nhấn Escape. */
  ...
  /** Cập nhật query lessonId để URL phản ánh bài đang học. */
  ...
  /** Chọn lesson, lưu vị trí và đồng bộ URL cho người học. */
  ...
  /** Đảo trạng thái hoàn thành lesson với optimistic update và rollback khi API lỗi. */
  ...
  /** Chỉ chấp nhận đường dẫn nội bộ làm nơi quay lại để tránh open redirect. */
  ...
  /** Rời workspace và quay về màn hình nội bộ an toàn đã được xác định. */
...
/** Hiển thị một section có thể thu gọn và trạng thái hoàn thành của từng lesson. */

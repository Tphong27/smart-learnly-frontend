...
export function normalizeVideoAiContent(value) {
  ...
  return {
    ...
    status: String(
      content.status ?? content.contentStatus ?? "DRAFT",
    ).toUpperCase(),
    language: content.language ?? content.sourceLanguage ?? "vi",
    suggestedTitle: content.suggestedTitle ?? content.titleSuggestion ?? "",
...
function createAuthoringService(basePath) {
  return {
    ...
    async retryJob(jobId) {
      return normalizeJob(
        await apiClient.post(`${basePath}/jobs/${jobId}/retry`),
      );
    },
    async getCurrentContent() {
      return normalizeVideoAiContent(
        await apiClient.get(`${basePath}/contents/current`),
      );
    },
    ...
    async publishContent(contentId, revision) {
      return normalizeVideoAiContent(
        await apiClient.post(`${basePath}/contents/${contentId}/publish`, {
          revision,
        }),
      );
    },
    async getFlashcardTargets() {
      const response = unwrap(
        await apiClient.get(`${basePath}/flashcard-targets`),
      );
    ...
    async createFlashcardJob(contentId, payload) {
      return normalizeJob(
        await apiClient.post(
          `${basePath}/contents/${contentId}/flashcard-jobs`,
          payload,
        ),
      );
    },
...
export const learnerVideoAiService = {
  ...
  async getTools(courseId, lessonId, classId, { signal } = {}) {
    const response = unwrap(
      await apiClient.get(
        `/learning/courses/${courseId}/lessons/${lessonId}/video-ai/tools`,
        { params: classId ? { classId } : {}, signal },
      ),
    );
    return {
      ...response,
      ...
      content: response?.content
        ? normalizeVideoAiContent(response.content)
        : null,
    };
  },
  async generateFlashcards(courseId, lessonId, classId, payload) {
    return unwrap(
      await apiClient.post(
        `/learning/courses/${courseId}/lessons/${lessonId}/video-ai/tools/flashcards`,
        payload,
        { params: classId ? { classId } : {} },
      ),
    );
  },
  async generateQuiz(courseId, lessonId, classId, payload) {
    return unwrap(
      await apiClient.post(
        `/learning/courses/${courseId}/lessons/${lessonId}/video-ai/tools/quiz`,
        payload,
        { params: classId ? { classId } : {} },
      ),
    );
